...
def run():
    ...
    class Net(nn.Module):
        ...
        def __init__(self):
            super().__init__()
            size = set.image_shape[0]
            self.conv1 = torch.nn.Conv2d(3, 128, kernel_size = 11, stride = 5)
            size = out_size(size, 11, 5)
            self.conv2 = torch.nn.Conv2d(128, 512, kernel_size = 5, stride = 2)
            size = out_size(size, 5, 2)
            self.pool1 = torch.nn.MaxPool2d(3)
            size = size // 3
            self.conv3 = torch.nn.Conv2d(512, 1024, kernel_size = 5, stride = 2)
            size = out_size(size, 5, 2)

            self.fc1 = nn.Linear(size*size*1024, 2048)
            ...
            for data in testset:
                ...
                for idx, i in enumerate(output):
                    if torch.argmax(i) == Y[idx]:
                        correct += 1
                    total += 1
            print("acc: ", round(correct/total* 100, 3), "%  cost: ", running_loss)
            running_loss = 0.0
